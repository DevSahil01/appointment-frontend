...
import {Box,Button,Flex,Heading,Image,Stack,Badge,Text,Link, Avatar,Center, Divider} from '@chakra-ui/react'
...
const Home = () => {
  ...
  const GetData=async ()=>{
      await axios.get('/api/user/get_sp')
      .then(res=>{
      ...
      return <Box
        ...
        <Stack p={4} direction={'row'} spacing={4} align={'center'}>
          ...
          <Button bg={'green.400'} color='white' my='4' w={'80%'} mx="5" _hover={
            {"color":"green",
        "backgroundColor":"gray.100"}}><Link onClick={()=>navigate('/serviceProvider/'+_id)}>Show Services</Link></Button>
      </Box>}):<Heading as={'h3'} >no services</Heading>}
    </Center>
    </Flex>
    <Divider/>
      ...
      return  <Box
       ...
       <Avatar
         ...
         <Button bg={'cyan.500'} color='white' my='4' w={'80%'} mx="5" _hover={
           {"color":"#00b5d8",
       "backgroundColor":"gray.100"}}><Link onClick={()=>navigate(`/appointment/${_id}`)}>Book Appointment</Link></Button>
     </Box>
    }):"nothing"}
    </Center>
